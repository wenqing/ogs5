      ...
      bbeta1(dynamic ? dm_pcs->m_num->GetDynamicDamping_bbeta() : 1.)
{
    ...
    const int max_ip_number = 64;
    Sxx = new double[max_ip_number];
    Syy = new double[max_ip_number];
    Szz = new double[max_ip_number];
    Sxy = new double[max_ip_number];
    Sxz = (dim == 3) ? new double[max_ip_number] : NULL;
    Syz = (dim == 3) ? new double[max_ip_number] : NULL;
    pstr = new double[max_ip_number];
    ...
    if (pcs->ite_steps == 1)
    {
            ...
            MeshElement->GetMark())
        {
                ...
                MeshElement->GetPatchIndex())
            {
                ...
                {
                    excavation = true;
                    // The released load is purely generated by the initial
                    // total stress
                    *(eleV_DM->Stress) = 0.;
                    // Pore pressure is set to what is set in the excavated
                    // domain by the Dirichlet BC
                }
            }
        }
    }
...
void CFiniteElementVec::LocalAssembly_continuum(const int update)
{
    ...
    const int PModel =
        (smat->CreepModel() != 21) ? smat->Plasticity_type : 9999;
    ...
    for (gp = 0; gp < nGaussPoints; gp++)
    {
        ...
        else
        {
            switch (PModel)
            {
                ...
                case 44:  // WX:12.2011 Mohr-Coloumb	bedding
                    ...
                    break;
                case 45:  // LU:12.2019 Mohr-Coloumb matrix and plane of
                          // weakness
        ...
        if (smat->Creep_mode == 21)  // BGRa with the implicit algorithm
        {
            ...
            smat->_bgra_creep->integrateStress(dt, temperature, *smat, *De,
                                               *ConsistDep, stress0, dstress,
                                               update);
            dPhi = 1.0;
        }
...
void CFiniteElementVec::ExtropolateGuassStrain()
{
    ...
    ip_data_vector.push_back(Eigen::Map<Eigen::VectorXd>(Sxx, nGaussPoints));
    idxs.push_back(_nodal_strain_indices[0]);
    ip_data_vector.push_back(Eigen::Map<Eigen::VectorXd>(Syy, nGaussPoints));
    idxs.push_back(_nodal_strain_indices[1]);
    ip_data_vector.push_back(Eigen::Map<Eigen::VectorXd>(Szz, nGaussPoints));
    idxs.push_back(_nodal_strain_indices[2]);
    ip_data_vector.push_back(Eigen::Map<Eigen::VectorXd>(Sxy, nGaussPoints));
    idxs.push_back(_nodal_strain_indices[3]);

    if (ele_dim == 3)
    {
        ip_data_vector.push_back(
            Eigen::Map<Eigen::VectorXd>(Sxz, nGaussPoints));
        idxs.push_back(_nodal_strain_indices[4]);
        ip_data_vector.push_back(
            Eigen::Map<Eigen::VectorXd>(Syz, nGaussPoints));
        idxs.push_back(_nodal_strain_indices[5]);
    }
    ...
    for (std::size_t ipd = 0; ipd < ip_data_vector.size(); ipd++)
    {
        ...
        for (int i = 0; i < nnodes; i++)
        {
            double EV = extrapolated_data[i] / dbuff[i];
            EV += pcs->GetNodeValue(nodes[i], idxs[ipd]);
            pcs->SetNodeValue(nodes[i], idxs[ipd], EV);
        }
    }
}
...
void CFiniteElementVec::ExtropolateGuassStress()
{
    ...
    for (gp = 0; gp < nGaussPoints; gp++)
    {
        Sxx[gp] = (*eleV_DM->Stress)(0, gp);
        Syy[gp] = (*eleV_DM->Stress)(1, gp);
        Szz[gp] = (*eleV_DM->Stress)(2, gp);
        Sxy[gp] = (*eleV_DM->Stress)(3, gp);
        if (eleV_DM->pStrain)
            pstr[gp] = (*eleV_DM->pStrain)(gp);
        else
            pstr[gp] = 0.0;  // 08.02.2008 WW
        if (ele_dim == 3)
        {
            Sxz[gp] = (*eleV_DM->Stress)(4, gp);
            Syz[gp] = (*eleV_DM->Stress)(5, gp);
        }
    }
    ...
    ip_data_vector.push_back(Eigen::Map<Eigen::VectorXd>(Sxx, nGaussPoints));
    idxs.push_back(_nodal_stress_indices[0]);
    ip_data_vector.push_back(Eigen::Map<Eigen::VectorXd>(Syy, nGaussPoints));
    idxs.push_back(_nodal_stress_indices[1]);
    ip_data_vector.push_back(Eigen::Map<Eigen::VectorXd>(Szz, nGaussPoints));
    idxs.push_back(_nodal_stress_indices[2]);
    ip_data_vector.push_back(Eigen::Map<Eigen::VectorXd>(Sxy, nGaussPoints));
    idxs.push_back(_nodal_stress_indices[3]);

    if (ele_dim == 3)
    {
        ip_data_vector.push_back(
            Eigen::Map<Eigen::VectorXd>(Sxz, nGaussPoints));
        idxs.push_back(_nodal_stress_indices[4]);
        ip_data_vector.push_back(
            Eigen::Map<Eigen::VectorXd>(Syz, nGaussPoints));
        idxs.push_back(_nodal_stress_indices[5]);
    }
