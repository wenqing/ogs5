...
// TODO
    ...
    {
        excavation = false;
        if ((smat->excavation > 0 || !pcs->ExcavMaterialGroup.empty()) &&
            MeshElement->GetMark())
        {
            int valid;
            ...
            for (std::size_t i_exc_mat = 0;
                 i_exc_mat < pcs->ExcavMaterialGroup.size(); i_exc_mat++)
            {
                if (static_cast<size_t>(pcs->ExcavMaterialGroup[i_exc_mat]) ==
                    MeshElement->GetPatchIndex())
                {
                    ...
                    if (pcs->isPointInExcavatedDomain(ele_center,
    ...
    if (excavation)
    {
        ...
        for (int i = 0; i < nnodesHQ; i++)
        {
            ...
            for (std::size_t j = 0; j < n_elements; j++)
            {
                ...
                else if (!pcs->ExcavMaterialGroup.empty())
                {
                    double const* ele_center(elem->GetGravityCenter());
                    ...
                    {
                        for (std::size_t i_exc_mat = 0;
                             i_exc_mat < pcs->ExcavMaterialGroup.size();
                             i_exc_mat++)
                        {
                            if (elem->GetPatchIndex() !=
                                static_cast<size_t>(
                                    pcs->ExcavMaterialGroup[i_exc_mat]))
                            {
                                onExBoundary = true;
                                break;
                            }
...
void CFiniteElementVec::ExtropolateGuassStrain()
{
    ...
    if (!pcs->ExcavMaterialGroup.empty())
    {
