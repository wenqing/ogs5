    ...
    switch (PcsType)
    {
        ...
        {  // Since there might be mass lumping, the IP data have to be
           // calculated.
            ...
            double density_args[] = {std::max(PG, 0.0), TG, 0.0};
            rhow = FluidProp->Density(density_args);

            const double drho_dp_rho =
                FluidProp->compressibility_model_pressure > 0
                    ? FluidProp->drhodP(density_args) / rhow
                    : FluidProp->drho_dp;
...
void CFiniteElementStd::CalCoefLaplace(bool Gravity, int ip)
{
    ...
    switch (PcsType)
    {
        ...
        case EPT_RICHARDS_FLOW:  // Richards flow
        {
            ...
            {
                rhow = val_ip.rho_w;  // FluidProp->Density();
                poro = val_ip.poro;   // MediaProp->Porosity(Index,
                                     // pcs->m_num->ls_theta);
                tort =
                    val_ip.tort;  // MediaProp->TortuosityFunction(Index, unit,
                                  //                  pcs->m_num->ls_theta);
                //
                Dpv = val_ip.Dv;
                // MediaProp->base_heat_diffusion_coefficient * tort *
                //       (1 - Sw) * poro *
                //       pow(TG / PhysicalConstant::CelsiusZeroInKelvin, 1.8);
                Dpv *= time_unit_factor * val_ip.rho_gw /
...
void CFiniteElementStd::CalcValuesAtIntegrationPoint(
    const bool is_pressure_primary_variable)
{
    double args[3];
    // Loop over Gauss points
    for (int gp = 0; gp < nGaussPoints; gp++)
    {
        ...
        val_ip.S_w = val_ip.p < 0.0
                         ? MediaProp->SaturationCapillaryPressureFunction(-PG)
                         : 1.0;
        ...
        val_ip.humidity = exp(std::min(PG, 0.0) /
                              (SpecificGasConstant::WaterVapour * TG * rhow));

        val_ip.Dvp = MediaProp->base_heat_diffusion_coefficient * (1 - Sw) *
                     std::pow(TG / PhysicalConstant::CelsiusZeroInKelvin, 1.8);

        val_ip.Dv = tort * poro * val_ip.Dvp;
        val_ip.rho_gw = val_ip.humidity * FluidProp->vaporDensity(TG);
    }
}
...
void CFiniteElementStd::CalcRHS_by_ThermalDiffusion()
{
    ...
    for (gp = 0; gp < nGaussPoints; gp++)
    {
        ...
        const double rhow = val_ip.rho_w;  // FluidProp->Density();
        const double PG = val_ip.p;        // interpolate(NodalVal1);
        const double TG = val_ip.T;        // interpolate(NodalValC);
        // WW
        const double Sw =
            val_ip.S_w;  // MediaProp->SaturationCapillaryPressureFunction(-PG);
        const double humi =
            val_ip.humidity;  // exp(PG / (SpecificGasConstant::WaterVapour * TG
                              // * rhow));
        const double Dv = val_ip.Dv;
        // MediaProp->base_heat_diffusion_coefficient * tort * (1 - Sw) *
        //      poro * pow(TG / PhysicalConstant::CelsiusZeroInKelvin, 1.8);
        const double rho_gw =
            val_ip.rho_gw;  // humi * FluidProp->vaporDensity(TG);
        const double drdT =
            (FluidProp->vaporDensity_derivative(TG) * humi -
             rho_gw * PG /
                 (SpecificGasConstant::WaterVapour * rhow * TG * TG)) /
            rhow;
        const double Dtv = time_unit_factor * Dv * drdT;
...
void CFiniteElementStd::Assemble_RHS_LATENT_HEAT_TRANSPORT()
{
    ...
    for (int gp = 0; gp < nGaussPoints; gp++)
    {
        ...
        const double dp_dt =
            (vvar_buffer.p - interpolate(NodalValC)) / (dt * time_unit_factor);
        const double rho_w = vvar_buffer.rho_w;

        args[0] = std::max(0.0, vvar_buffer.p);
        args[1] = std::max(0.0, vvar_buffer.T);
        args[2] = 0.0;
