...
public:
    ...
    void multi(const MatrixBase& m, MatrixBase& m_result, double fac = 1.0);
    // m_result = this*m1*m2. m_result must be initialized.To be removed
    void multi(const MatrixBase& m1, const MatrixBase& m2,
               MatrixBase& m_result);
...
public:
    ...
    // vec_result = This*vec. vec_result must be initialized.
    void multi(const double* vec, double* vec_result,
               double fac = 1.0) override;
    // m_result = this*m. m_result must be initialized.
    ...
    // Access to members
    double& operator()(const size_t i, const size_t j = 0) const override;
...
public:
    ...
    // Access the element
    inline double& operator()(const size_t i, const size_t j = 1) const override
    {
    ...
    // stored matrix
    void multi(const SymMatrix& m, Matrix& m_result, double fac = 1.0);
    // m_result = this*m1*m2. m_result must be initialized.  m_result must be a
    // full stored matrix
    void multi(const SymMatrix& m1, const Matrix& m2, Matrix& m_result);
